...
func (h *OSAAppsHandler) ListApps(c *gin.Context) {
	...
	// Get apps
	apps, err := h.queries.ListOSAGeneratedAppsByUser(c.Request.Context(), sqlc.ListOSAGeneratedAppsByUserParams{
		UserID:  pgUserID,
	...
	// Get total count
	totalCount, err := h.queries.CountOSAGeneratedAppsByUser(c.Request.Context(), sqlc.CountOSAGeneratedAppsByUserParams{
		UserID:  pgUserID,
		Column2: pgWorkspaceID,
		Column3: statusStr,
	})
...
func (h *OSAAppsHandler) GetApp(c *gin.Context) {
	...
	app, err := h.queries.GetOSAGeneratedAppByIDWithAuth(c.Request.Context(), sqlc.GetOSAGeneratedAppByIDWithAuthParams{
		ID:     pgAppID,
		UserID: pgUserID2,
	})
	if err != nil {
...
func (h *OSAAppsHandler) DeleteApp(c *gin.Context) {
	...
	pgAppID := pgtype.UUID{Bytes: appID, Valid: true}
	pgUserID := pgtype.UUID{Bytes: userID, Valid: true}
	_, err = h.queries.GetOSAGeneratedAppByIDWithAuth(c.Request.Context(), sqlc.GetOSAGeneratedAppByIDWithAuthParams{
		ID:     pgAppID,
		UserID: pgUserID,
	})
	if err != nil {
	...
	err = h.queries.DeleteOSAGeneratedApp(c.Request.Context(), pgAppID)
	if err != nil {
...
func (h *OSAAppsHandler) UpdateApp(c *gin.Context) {
	...
	// Update app
	updatedApp, err := h.queries.UpdateOSAGeneratedAppMetadata(c.Request.Context(), sqlc.UpdateOSAGeneratedAppMetadataParams{
		ID:          pgAppID,
