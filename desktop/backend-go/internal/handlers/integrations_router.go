...
func NewIntegrationRouter(pool *pgxpool.Pool) *IntegrationRouter {
	...
	if err != nil {
		slog.Warn("[IntegrationRouter] Failed to create google_calendar handler", "error", err)
	}
	googleGmailHandler, err := google.NewToolHandler(pool, "google_gmail")
	if err != nil {
		slog.Warn("[IntegrationRouter] Failed to create google_gmail handler", "error", err)
	}
