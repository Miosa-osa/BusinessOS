defmodule OptimalEngine.Bridge.Signal do
  @moduledoc """
  Bridge to MiosaSignal — CloudEvents envelopes, classification, and quality auditing.

  MiosaSignal is stateless (no GenServers). All functions are pure wrappers.

  ## Integration points

  - `classify/1` — Augments OptimalEngine.Classifier with MiosaSignal.Classifier's
    inference (mode, genre, type, format, structure) for improved accuracy
  - `audit/1` — Runs MiosaSignal.FailureModes.detect/1 against a signal to produce
  ...
  def audit(%OptSignal{} = signal) do
    ...
    MiosaSignal.FailureModes.detect(event)
  rescue
    _ -> []
  end
  ...
  def measure_sn(%OptSignal{} = signal) do
    ...
    case MiosaSignal.measure_sn_ratio(miosa_signal) do
  ...
  def to_cloud_event(%OptSignal{} = signal) do
    ...
    case MiosaSignal.to_cloud_event(miosa_signal) do
      %{} = event -> event
      _ -> %{}
    end
  rescue
  ...
  # Convert to MiosaSignal.Event struct (for Classifier + FailureModes)
  ...
  # Convert OptimalEngine.Signal to MiosaSignal struct (CloudEvents envelope)
  defp to_miosa_signal(%OptSignal{} = signal) do
    MiosaSignal.new(%{
      ...
      signal_type: normalize_type(signal.type),
      signal_format: normalize_format(signal.format),
      signal_structure: if(is_binary(signal.structure), do: String.to_atom(signal.structure), else: nil),
