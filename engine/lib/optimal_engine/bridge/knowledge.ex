defmodule OptimalEngine.Bridge.Knowledge do
  @moduledoc """
  Bridge to MiosaKnowledge — stateless graph queries via SQLite, on-demand OWL reasoning.
  ...
  OWL reasoning (`materialize/0`) is on-demand only: lazily opens a MiosaKnowledge
  ...
  - `materialize/0` — Syncs edges to MiosaKnowledge, runs OWL 2 RL, returns inferred count
  ...
  @doc """
  Syncs SQLite edges into a temporary MiosaKnowledge store and runs OWL 2 RL
  ...
  @doc """
  Syncs edges from SQLite to MiosaKnowledge ETS store, then runs OWL reasoning.
  ...
  # ---------------------------------------------------------------------------
  # Private: On-demand MiosaKnowledge store
  # ---------------------------------------------------------------------------

  defp open_knowledge_store do
    MiosaKnowledge.open(@store_name, backend: MiosaKnowledge.Backend.ETS)
  end
  ...
  defp run_materialization(store) do
    MiosaKnowledge.Reasoner.materialize(store, store)
  end
end
