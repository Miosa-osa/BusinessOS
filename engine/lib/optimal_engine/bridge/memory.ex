defmodule OptimalEngine.Bridge.Memory do
  @moduledoc """
  Bridge to MiosaMemory — episodic events, SICA learning, Cortex synthesis, and injection.

  MiosaMemory's GenServers (Store, Episodic, Cortex, Learning) are started in
  ...
  def record_event(event_type, data, session_id \\ "optimal-engine") do
    MiosaMemory.Episodic.record(event_type, data, session_id)
    :ok
  rescue
    _ -> :ok
  end
  ...
  def recall_events(query, opts \\ []) do
    case MiosaMemory.Episodic.recall(query, opts) do
  ...
  def observe_mutation(interaction) when is_map(interaction) do
    MiosaMemory.Learning.observe(interaction)
    :ok
  rescue
    _ -> :ok
  end
  ...
  def record_correction(original, corrected) do
    MiosaMemory.Learning.correction(original, corrected)
    :ok
  rescue
    _ -> :ok
  end
  ...
  def record_error(tool_name, error_message, context \\ %{}) do
    MiosaMemory.Learning.error(tool_name, error_message, context)
    :ok
  rescue
    _ -> :ok
  end
  ...
  @spec solutions() :: map()
  def solutions do
    MiosaMemory.Learning.solutions()
  rescue
    _ -> %{}
  end
  ...
  def bulletin do
    case MiosaMemory.Cortex.bulletin() do
  ...
  def active_topics do
    case MiosaMemory.Cortex.active_topics() do
  ...
  def inject_context(entries, context) when is_list(entries) and is_map(context) do
    case MiosaMemory.Injector.inject_relevant(entries, context) do
      selected when is_list(selected) ->
        MiosaMemory.Injector.format_for_prompt(selected)
  ...
  def remember(insight, opts \\ []) do
    MiosaMemory.remember(insight, opts)
    :ok
  rescue
    _ -> :ok
  end
  ...
  def search(query, opts \\ []) do
    case MiosaMemory.search(query, opts) do
