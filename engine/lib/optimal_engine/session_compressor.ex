defmodule OptimalEngine.SessionCompressor do
  ...
  defp llm_compress(text) do
    ...
    case Ollama.generate(prompt, system: system) do
      {:ok, compressed} ->
        Logger.debug("[SessionCompressor] LLM compression: #{String.length(text)} → #{String.length(compressed)} chars")
        {:ok, String.trim(compressed)}

      {:error, reason} ->
        Logger.info("[SessionCompressor] LLM unavailable (#{inspect(reason)}), using regex fallback")
        regex_compress(text)
    end
  rescue
    err ->
      Logger.warning("[SessionCompressor] llm_compress rescued: #{inspect(err)}, falling back to regex")
      regex_compress(text)
  end
  ...
  defp regex_compress(text) do
    ...
    Logger.debug("[SessionCompressor] Regex compression: #{String.length(text)} → #{String.length(final)} chars")
    {:ok, final}
  rescue
    err ->
  ...
  defp high_signal_assistant?(line) do
    ...
    contains_date = String.match?(line, ~r/\b\d{4}[-\/]\d{2}[-\/]\d{2}\b|\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/i)
