defmodule OptimalEngine.Simulator do
  ...
  defp regex_parse_scenario(scenario, downcased) do
    cond do
      ...
      Regex.match?(~r/\bleav(e|es|ing)\b|\bdeparture\b|\bwithout\b|\bfire[sd]?\b|\bremove[sd]?\b/, downcased) ->
      ...
      Regex.match?(~r/\bcancel\b|\bshut down\b|\bdrop\b|\bend\b|\bkill\b|\bclose\b|\bdissolve\b/, downcased) ->
  ...
  defp assess_risk(scored_nodes, critical_deps, traversal) do
    ...
    reasoning = build_risk_reasoning(severity, total_affected, nodes_affected, critical_count, revenue_affected)
  ...
  defp fallback_recommendations(%{type: :dependency_break, entity: entity, node: node}, scored_nodes, _deps) do
    top_nodes = scored_nodes |> Enum.take(3) |> Enum.map(& &1.node)
