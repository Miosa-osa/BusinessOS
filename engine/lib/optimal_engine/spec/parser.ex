defmodule OptimalEngine.Spec.Parser do
  ...
  def parse_string(content) when is_binary(content) do
    ...
    spec = %{
      ...
      requirements: parse_list(blocks["spec-requirements"], &normalize_requirement/1, "spec-requirements"),
      scenarios: parse_list(blocks["spec-scenarios"], &normalize_scenario/1, "spec-scenarios"),
      verifications: parse_list(blocks["spec-verification"], &normalize_verification/1, "spec-verification"),
  ...
  defp parse_meta(nil), do: %{id: nil, kind: nil, status: "draft", surface: [], verification_minimum_strength: "claimed"}
  defp parse_meta({:error, _}), do: %{id: nil, kind: nil, status: "draft", surface: [], verification_minimum_strength: "claimed"}
  ...
  defp normalize_requirement(_), do: %{id: nil, statement: "", priority: "should", stability: "evolving"}
  ...
  defp normalize_scenario(_), do: %{id: nil, covers: [], given: [], when_clause: [], then_clause: []}
