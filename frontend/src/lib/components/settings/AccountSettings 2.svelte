		...
		<div class="flex items-start gap-6">
			...
			<div class="flex-shrink-0">
				<div class="relative">
					...
					{/if}
					<label class="absolute bottom-0 right-0 w-8 h-8 bg-gray-900 dark:bg-white rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors" aria-label="Upload profile photo">
						{#if isUploadingPhoto}
			...
			<div class="flex-1 space-y-4">
				{#if isEditingProfile}
					<div>
						<label for="edit-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Display Name</label>
						<input
							id="edit-name"
							type="text"
							bind:value={editName}
				...
				{:else}
					<div>
						<p class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Name</p>
						<p class="text-gray-900 dark:text-white">{$session.data?.user?.name || 'Not set'}</p>
					</div>
					<div>
						<p class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Email</p>
					...
					<div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-sm text-gray-500 dark:text-gray-400">{activeSession.location} • {activeSession.lastActive}</p>
							</div>
						</div>
						{#if !activeSession.current}
							<button
