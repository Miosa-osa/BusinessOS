...
export class OSAVoiceService {
	...
	async speak(text: string): Promise<boolean> {
		...
		if (import.meta.env.DEV) console.log(`[OSA Voice] Queued: "${text.substring(0, 50)}..." (Queue size: ${this.audioQueue.length})`);
	...
	private async processQueue() {
		...
		while (this.audioQueue.length > 0) {
			...
			if (import.meta.env.DEV) console.log(`[OSA Voice] Speaking: "${text.substring(0, 50)}..."`);
			try {
		...
		if (this.audioQueue.length > 0) {
			if (import.meta.env.DEV) console.log('[OSA Voice] More items added, continuing queue');
			this.processQueue();
		} else {
			if (import.meta.env.DEV) console.log('[OSA Voice] Queue complete');
		}
	}
	...
	private async speakNow(text: string): Promise<void> {
		try {
			if (import.meta.env.DEV) console.log('[OSA Voice] Requesting TTS', { text_length: text.length });
			...
			if (import.meta.env.DEV) console.log('[OSA Voice] Audio received', { size_bytes: audioBlob.size });
	...
	private async playAudio(audioUrl: string): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			if (import.meta.env.DEV) console.log('[OSA Voice] ▶️ Playing audio');

			// Handle playback events
			audio.onended = () => {
				if (import.meta.env.DEV) console.log('[OSA Voice] ⏹️ Audio ended');
	...
	stop() {
		...
		if (import.meta.env.DEV) console.log('[OSA Voice] Stopped');
	}
