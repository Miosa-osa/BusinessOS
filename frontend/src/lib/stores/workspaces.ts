...
export async function initializeWorkspaces(): Promise<void> {
  ...
  try {
    ...
    if (import.meta.env.DEV) {
      if (import.meta.env.DEV) console.log("[Workspaces] API failed in dev mode, loading mock data");
      loadMockWorkspaceData();
      workspaceError.set(null);
    }
  } finally {
...
export function loadMockWorkspaceData(): void {
  if (import.meta.env.DEV) console.log("[Workspaces] Loading mock data for development");
  ...
  if (import.meta.env.DEV) console.log("[Workspaces] Mock data loaded - workspace:", mockWorkspace.name);
}
