<script lang="ts">
	...
	// Health config
	const healthConfig: Record<NodeHealth, { color: string; bgColor: string; label: string }> = {
		...
		<div class="border-b border-gray-200 px-6 py-4 flex-shrink-0">
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-4">
					<div class="w-12 h-12 rounded-xl bg-{nodeTypeConfig[node.type].color}-100 text-{nodeTypeConfig[node.type].color}-600 flex items-center justify-center">
					...
					<div>
						...
						<div class="flex items-center gap-3 mt-1">
							<span class="text-sm text-gray-500 capitalize">{nodeTypeConfig[node.type].label} Node</span>
							<span class="text-gray-300">|</span>
							<span class="flex items-center gap-1.5 text-sm {healthConfig[node.health].color}">
								<span class="w-2 h-2 rounded-full {healthConfig[node.health].bgColor}"></span>
								{healthConfig[node.health].label}
							</span>
							<span class="text-gray-300">|</span>
						...
						<div class="flex items-center justify-between mb-3">
							<div class="flex items-center gap-3">
								...
								<select
									...
									class="text-sm px-2 py-1 border border-gray-200 rounded-lg {healthConfig[node.health].color} focus:outline-none focus:ring-2 focus:ring-blue-500"
								>
					...
					<div class="bg-white border border-gray-200 rounded-xl p-5">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
							{#each children as child}
								...
								>
									<div class="w-9 h-9 rounded-lg bg-{nodeTypeConfig[child.type].color}-100 text-{nodeTypeConfig[child.type].color}-600 flex items-center justify-center flex-shrink-0">
										<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={nodeTypeConfig[child.type].icon} />
										</svg>
									</div>
									<div class="flex-1 min-w-0">
										<p class="text-sm font-medium text-gray-900 truncate">{child.name}</p>
										<span class="flex items-center gap-1 text-xs text-gray-500">
											<span class="w-1.5 h-1.5 rounded-full {healthConfig[child.health].bgColor}"></span>
											{healthConfig[child.health].label}
										</span>
									</div>
								</a>
							{/each}
