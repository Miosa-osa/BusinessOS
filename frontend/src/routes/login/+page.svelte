	...
	async function handleSubmit(e: Event) {
		...
		try {
			if (import.meta.env.DEV) {
				console.log('[LOGIN DEBUG] Attempting sign-in with email:', email, 'password length:', password.length);
			}
			...
			// Refresh the session store with real user data from backend
			await refreshSession();
			// It won't appear in document.cookie - this is correct security behavior
			// The browser sends it automatically with HTTP requests
			// If the backend returned success, the cookie was set correctly
			...
			if (isInIframe) {
				// If in iframe, don't navigate - user is already on /window in parent window
				loading = false;
				// Send message to parent that login succeeded
				if (window.parent) {
...
<AuthLayout>
	<div in:fly={{ y: 20, duration: 400 }}>
				...
				<div class="flex items-center gap-3 mt-3 p-3 bg-gray-50 rounded-lg border border-gray-100">
					...
					<div class="flex-1 min-w-0">
						...
						<button
							...
							class="btn-pill btn-pill-ghost btn-pill-xs font-mono"
						>
		...
		<form onsubmit={handleSubmit} class="space-y-5">
			...
			<button
				type="submit"
				class="btn-pill btn-pill-primary btn-pill-sm w-full h-12 flex items-center justify-center gap-2 font-mono"
				disabled={loading}
			>
				{#if loading}
		...
		<button
			...
			class="btn-pill btn-pill-secondary btn-pill-sm w-full h-12 flex items-center justify-center gap-3"
		>
